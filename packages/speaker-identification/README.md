...
The interactive UI is the `/speaker-identification` route in [`packages/sandbox`](../sandbox). It is linked from the sandbox home page. See its README for model preparation, recording controls, and UI tests.
...
UI, sample-management, and recorder regressions belong to the sandbox:
...
The sandbox separates Worker RPC, sample/centroid management, capture, and score rendering under `src/features/speaker-identification/`. Fixture loading and timed recording live in `tests/speaker-identification/`. The recorder scales an entire clip down only if its floating-point peak exceeds one; invalid nonfinite values remain errors. Public extractor input is still normalized mono PCM.
