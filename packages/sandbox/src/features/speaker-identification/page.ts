...
/** Triggering workflow: sandbox route onMounted -> mountSpeakerPage -> bind this page's model, speaker and recording controls. */
export function mountSpeakerPage(root: HTMLElement) {
  const model = root.querySelector<HTMLSelectElement>('#model')!
  const load = root.querySelector<HTMLButtonElement>('#load')!
  const modelStatus = root.querySelector<HTMLParagraphElement>('#model-status')!
  const modelLoading = root.querySelector<HTMLDivElement>('#model-loading')!
  const modelProgress = root.querySelector<HTMLProgressElement>('#model-progress')!
  const enroll = root.querySelector<HTMLButtonElement>('#enroll')!
  const record = root.querySelector<HTMLButtonElement>('#record')!
  const speakerName = root.querySelector<HTMLInputElement>('#speaker-name')!
  const enrollStatus = root.querySelector<HTMLParagraphElement>('#enroll-status')!
  ...
  let activeRecording: Awaited<ReturnType<typeof startRecording>> | undefined
  ...
  function updateControls() {
    ...
    root.querySelector('#voice-count')!.textContent = String(registered.size)
  ...
  async function loadModel() {
    ...
    root.querySelector<HTMLElement>('#voices-empty')!.hidden = false
    modelLoading.hidden = false
    modelProgress.removeAttribute('value')
    modelStatus.classList.remove('error')
    root.querySelector('#model-badge')!.textContent = '加载中'
    root.querySelector('#model-badge')!.removeAttribute('data-ready')
    updateControls()
    try {
      ...
      root.querySelector('#model-badge')!.textContent = models[selected]!.name
      root.querySelector('#model-badge')!.setAttribute('data-ready', '')
    }
    catch (error) {
      modelStatus.textContent = errorMessage(error)
      modelStatus.classList.add('error')
      root.querySelector('#model-badge')!.textContent = '加载失败'
    }
    finally {
      loading = false
  ...
  function removeGroup(group: SpeakerGroup) {
    ...
    root.querySelector<HTMLElement>('#voices-empty')!.hidden = speakerGroups.size > 0
  }
  ...
  function getSpeakerGroup(name: string) {
    ...
    if (existing)
      return existing
    const template = root.querySelector<HTMLTemplateElement>('#speaker-template')!
    ...
    root.querySelector<HTMLElement>('#voices-empty')!.hidden = true
    return group
  }

  function createRow(kind: 'voice' | 'record', name: string) {
    const template = root.querySelector<HTMLTemplateElement>(`#${kind}-template`)!
    const element = template.content.firstElementChild!.cloneNode(true) as HTMLElement
    ...
    if (kind === 'record') {
      ...
      function deleteRecording() {
        if (remove.disabled)
          return
        element.remove()
        root.querySelector('#record-count')!.textContent = String(recordings.children.length)
        root.querySelector<HTMLElement>('#recording-empty')!.hidden = recordings.children.length > 0
      }
      remove.addEventListener('click', deleteRecording)
      recordings.prepend(element)
      root.querySelector<HTMLElement>('#recording-empty')!.hidden = true
      root.querySelector('#record-count')!.textContent = String(recordings.children.length)
    }
    else {
  ...
  root.querySelector('#enrollment-form')!.addEventListener('submit', event => enrollVoice(event as SubmitEvent))
  record.addEventListener('click', createRecording)
  updateControls()

  /** Triggering workflow: route onBeforeUnmount -> disposePage -> abort capture and release the page Worker. */
  return function disposePage() {
    disposed = true
    lifetime.abort()
    void activeRecording?.stop().catch(() => {})
    void speakerClient.dispose()
  }
}
