...
import { initVADASRModule } from '../src'
...
it('initializes VAD-ASR module or reports a clear prebuilt-missing error', async () => {
  try {
    const module = await initVADASRModule()
    expect(module).toBeDefined()
  }
  catch (error) {
    expect(String(error)).toContain('Missing prebuilt VAD-ASR module')
  }
}, 120000)
